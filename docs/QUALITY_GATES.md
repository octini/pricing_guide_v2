...
```bash
python3 scripts/reports/check_r2.py
```
...
```bash
#!/bin/bash
if [ -f "scripts/reports/check_r2.py" ]; then
    python3 scripts/reports/check_r2.py
    if [ $? -ne 0 ]; then
