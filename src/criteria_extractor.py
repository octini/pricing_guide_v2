...
def extract_entries_criteria(item: dict, prose_text: str = "") -> dict:
    ...
    def _classify_extra_damage_context(context_text: str) -> tuple[str, str | None, float]:
        ...
        for pattern in creature_patterns:
            creature_match = re.search(pattern, lower)
            if creature_match:
    ...
    def set_extra_damage_condition(context_text: str | None = None) -> None:
        if not c.get("extra_damage_avg"):
            return
        condition_text = (context_text or combined_text)
        condition, detail, multiplier = _classify_extra_damage_context(condition_text)
        c["extra_damage_condition"] = condition
        c["extra_damage_condition_detail"] = detail
        c["extra_damage_multiplier"] = multiplier
    ...
    damage_matches = list(re.finditer(r'(?:additional|extra) {@damage ([^}]+)}', combined_text))
    if damage_matches:
        total_avg = 0.0
        priced_total = 0.0
        conditions: list[str] = []
        ...
        for m in damage_matches:
            dmg_str = m.group(1)
            avg = _avg_dice(dmg_str)
            total_avg += avg
            ctx = extra_damage_context(m)
            cond, detail, mult = _classify_extra_damage_context(ctx)
            conditions.append(cond)
            details.append(detail)
            multipliers.append(mult)
            priced_total += avg * mult
        c["extra_damage_avg"] = total_avg
        c["extra_damage_priced_avg"] = priced_total
        c["extra_damage_dice"] = damage_matches[0].group(1) if len(damage_matches) == 1 else f"{len(damage_matches)} sources"
        unique_conds = set(conditions)
        if len(unique_conds) == 1:
            c["extra_damage_condition"] = conditions[0]
            # for vs_creature_type, preserve first non-None detail
            if conditions[0] == "vs_creature_type":
                first_detail = next((d for d in details if d), details[0])
                c["extra_damage_condition_detail"] = first_detail
            else:
                c["extra_damage_condition_detail"] = None
            c["extra_damage_multiplier"] = multipliers[0]
        else:
            c["extra_damage_condition"] = "mixed"
            c["extra_damage_condition_detail"] = None
    ...
    if not c.get('extra_damage_avg'):
        ...
        if plain_matches:
            ...
            c['extra_damage_avg'] = total_avg
            c['extra_damage_priced_avg'] = priced_total
            c['extra_damage_dice'] = plain_matches[0].group(1) if len(plain_matches) == 1 else f"{len(plain_matches)} sources"
            unique_conds = set(conditions)
            if len(unique_conds) == 1:
                c['extra_damage_condition'] = conditions[0]
                if conditions[0] == "vs_creature_type":
    ...
    if "Dragon's Wrath" in item_name or "Dragon\u2019s Wrath" in item_name:
        if "Slumbering" in item_name:
            ...
            c["extra_damage_priced_avg"] = 3.5 * 0.05
...
def extract_prose_criteria(description: str) -> dict:
    ...
    if _best_temp_span is not None:
        ...
        # Frequency classification: daily → on_kill → per_action, bound to grant's sentence
        _window_start = max(0, _s - 120)
        _window_end = min(len(desc), _e + 120)
        _window = desc[_window_start:_window_end]
        _match_rel_start = _s - _window_start
        _match_rel_end = _e - _window_start
        # Find sentence containing the match within window (split at . ; ! ?)
        _delims = ['.', ';', '!', '?']
        _last_delim = -1
        for _d in _delims:
            _p = _window.rfind(_d, 0, _match_rel_start)
            if _p > _last_delim:
                _last_delim = _p
        _sent_start = _last_delim + 1 if _last_delim != -1 else 0
        _next_delim = len(_window)
        for _d in _delims:
            _p = _window.find(_d, _match_rel_end)
            if _p != -1 and _p < _next_delim:
                _next_delim = _p
        _sent_end = _next_delim + 1 if _next_delim != len(_window) else len(_window)
        _sentence_window = _window[_sent_start:_sent_end]
    ...
    if _hp_max_match:
        ...
        if _m:
            # "N + your level" (incl. "an amount equal to N + your level") → flat N, per_level 1
            c["hp_max_flat"] = int(_m.group(1))
            c["hp_max_per_level"] = 1
        else:
            ...
            if _m:
                ...
                if _m:
                    # Standalone "an amount equal to N" (no level) → flat N
                    c["hp_max_flat"] = int(_m.group(1))
                else:
                    _m = re.search(r'(\d+d\d+(?:\s*\+\s*\d+)?)', _captured)
                    if _m:
                        # Dice forms like "1d4" or "2d10 + 4" → flat avg, keep float
                        c["hp_max_flat"] = float(_avg_dice(_m.group(1)))
                    else:
    ...
    if _init_match:
        try:
            ...
            pass
    if re.search(r'(?<!dis)\badvantage on (?:all )?initiative', desc):
